...
def check(source_currency, destination_currency, amount, prushpare_price):
    last_updated_datetime, exchange_rate ,currency_price = convert_currency_xe(source_currency, destination_currency, amount)
    ...
    if new_price > prushpare_price:
        print(f"The value increased by {percentage_change:.2f}%")
    elif new_price < prushpare_price:
        print(f"The value decreased by {percentage_change:.2f}%")
    else:
        print("The value is fixed")

    up_value = round(exchange_rate - (prushpare_price * amount))
    print(f"Your currency increased by {up_value} {destination_currency}")
